  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async (e) => {
    ...
    if (Validateemail(userInfo.email)) {
      ...
      try {
        ...
        const data: API_signinres | API_ERROR = await (
        ...
        if (data.error) {
          if (data.message === "Account Do Not Exists") {
            ...
            toast.error(data.message, {
              description:
                data.message === "Verify Your Account to Login" ? (
                  <button
  ...
  const sendverificationmail = () => {
    const Res = new Promise(async (resolve, reject) => {
      const re = await fetch(
        `/api/ResendVerificationUrl?email=${userInfo.email}`,
      );
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className={"flex flex-col  gap-10  w-full"}>
        ...
        <div className={"flex gap-3 justify-center items-center"}>
          <span
            className={"h-[.5px] w-full bg-[var(--secondary-color)]"}
          ></span>
          <p>OR</p>
          <span
            className={"h-[.5px] w-full bg-[var(--secondary-color)]"}
        ...
        <div className={"flex justify-evenly gap-5"}>
          <button
            type={"button"}
            className={
              "p-2 border rounded-xl w-full flex justify-center items-center gap-3"
            }
            onClick={Googlelogin}
          >
            <Image
