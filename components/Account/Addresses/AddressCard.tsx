...
export const AddressSection: FunctionComponent<{ addresses: address[], token: string }> = ({addresses, token}) => {
    const [address, setAddress] = useState<address[]>(addresses);
    const deleteaddress = async (addressid: string) => {
        const deleteaddress = gqlClient.request(DeleteAddressDocument, {
            id: addressid
        }, {
        ...
        <ul className={"grid grid-cols-2 gap-2 max-md:grid-cols-1"}>
            {
                address.map(e => {
                    return <AddressCard
                        token={token}
                        address={e} key={e.address_id + Math.random()} deleteaddress={deleteaddress}/>
...
const AddressCard: FunctionComponent<typesforAddressCard> = (props) => {
    ...
    return <section className={"border border-gray-500 p-4 flex flex-col justify-between gap-4 rounded-xl"}>
    ...
    return <section className={"flex flex-col gap-2"}>
        ...
        <div className="grid grid-cols-2 gap-4 h-min">
            <button type={"button"}
                    ...
                    className={"p-3 bg-secondary disabled:brightness-50 text-primary rounded-lg"}>
                Delete Address
            </button>
            <button type={"button"} className={"p-3 bg-secondary disabled:brightness-50 text-primary rounded-lg"}
                    onClick={() => {
