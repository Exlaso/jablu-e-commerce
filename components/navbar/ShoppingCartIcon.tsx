...
const ShoppingCartIcon = ({ token, isAuthed }: ShoppingCartIconProps) => {
    ...
    if (isAuthed) {
      ...
      const unsub = wsClient.subscribe(
        ...
        },
        {
          ...
          error() {
            console.error("Error in Cart WS - ");
          },
          complete() {
            console.info("Completed Cart WS");
          },
        },
      );
  ...
  return (
    <Link href={"/ShoppingBag"} className={""}>
      <span
        className={" flex justify-end items-end border-green-600 h-full w-full"}
      >
        <Badge content={Noofitemsincart} color="cyan">
