    ...
    return <motion.form
        ...
        <div className={"w-full rounded-lg border border-gray-500/50 p-3 flex flex-col gap-3"}>
            <div className={"w-full grid grid-cols-[.8fr,2fr,.5fr] gap-3"}>
                ...
                className={"grow-0 shrink-0 text-highlight  flex justify-center items-center"}>{phoneno}</span>
                <Link href={"./Address"}
                      className={"grow-0 shrink-0 text-sm text-blue-500 underline"}>Change</Link>
            </div>
            <hr className={"border border-gray-500/50"}/>
            <div className={"w-full grid grid-cols-[.8fr,2fr,.5fr] gap-3"}>
                <span className={"grow-0 shrink-0"}>Shipping Address</span><span
                className={"grow-0 shrink-0 text-highlight  flex justify-center items-center"}>{
                address1 + ", " + address2 + ", " + city + " - " + pincode + ", " + regionstate + ", " + region
        ...
        <div className={"flex flex-col gap-2"}>
            ...
            <fieldset className={"first:rounded-t-2xl last:rounded-b-2xl"}>
                {Shippingmethods.map(e => {
                    ...
                    return <label key={e.title} htmlFor={e.title}
                        ...
                        <div className={"flex gap-2"}>
                            <input
                                ...
                                type="radio" id={e.title} onChange={() => {
                                setShippingmethod(e)
                            }} name={"shippingmethods"}/>
                            <h3 className={"text-lg font-semibold  text-highlight"}>{e.title}</h3></div>
                        <div className={"flex justify-between "}>
                            <p>{e.description}</p>
                            <span className={"text-highlight font-bold"}>Rs. {e.price}
                        </span>
                        </div>
                    </label>
                })}
        ...
        <div className={"flex justify-between gap-4"}>
            <Link href={"./Address"} className={"text-blue-400 shrink-0 underline underline-offset-2"}> &lt; Return to
                information</Link>
            <button className={" p-3 rounded-lg bg-secondary grow-0 text-primary"}
                    type={"submit"}
            >
                Continue to Payment
            </button>
