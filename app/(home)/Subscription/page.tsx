...
const page: FunctionComponent<typesforpage> = async (props) => {
  ...
  const subscribedata = await gqlClient
    .request(
      GetSubscribedataDocument,
      {
        email: email,
      },
      {
  ...
  return (
    <main
      className={
        "py-[13vh] min-h-screen flex flex-col justify-center items-center"
      }
    >
      <BackButton />
      <div className={"flex flex-col justify-start w-full items-center gap-4"}>
        <h1 className={"text-4xl font-bold"}>Unsubscribe from Newsletter</h1>
