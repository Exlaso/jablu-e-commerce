  ...
  try {
    ...
    if (res.verificationurls.length === 0) {
      ...
      const emailsenddata: Emailsendinterface = {
        to: email as string,
        Body: {
          Firstname: res.verificationurls.at(0)?.user_verify
      ...
      const emailres = await fetch(
        `${process.env.NEXTAUTH_URL}/api/emailsend`,
        {
          method: "POST",
          headers: {
      ...
      const createemailres = await emailres.json();
      if (createemailres.data?.id) {
        return NextResponse.json({
          message: `Verification Mail is Successfully Sent to ${email}`,
          error: false,
        });
      } else {
        return NextResponse.json({
